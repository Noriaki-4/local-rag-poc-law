...
| `AWS-015` | P0 | 検証待ち | GenUのmodel選択とLegal AgentのLLM providerを対応付ける | AWS adapterにBedrock Converse接続を実装し、生成modelは環境設定のJapan geo inference profileへRuntime固定とした。GenU requestのmodel値は監査ログだけに使い、任意modelへ切り替えない。実modelでのstructured JSON互換性は未確認 | main側のLLM契約を壊さずBedrockを選択でき、許可model・regionをbackendで制約し、GenUの選択を採用するかRuntime固定にするかが契約とテストで明示されている |
| `AWS-013` | P0 | 対応中 | AWS環境の差分と構成変更を安全に扱う | schema version 8の環境JSONへNeptune、Bedrock、embedding入力上限、bootstrap imageを追加し、未知field・account・region・CIDR・AgentCore対応AZ IDを検証する。resource置換時のデータ移行とrollback手順は未実装 | 再利用IaC、環境別設定、deploy outputが分離され、別設定のplan検証、resource置換時のデータ移行・疎通確認・rollbackを追跡できる |
...
| `AWS-003` | P0 | 検証待ち | AWS基盤の最小IaCを作る | 5 stackへVPC、S3、ECS / ECR、AgentCore、one-off管理task、private OpenSearch Serverless、Neptune Analytics graph・private endpointを実装した。offline synthは合格したがAWSへのimage push、diff / deployは未実施 | network、data、compute、management、runtimeに必要な最小リソースを再現可能に作成・更新・削除でき、命名と共通tagが統一されている |
| `AWS-004` | P0 | 検証待ち | Legal AgentをAWSのOpenSearchへ安全に接続する | SigV4（`aoss`）Runtime adapter、Titan query embedding、kuromoji / ICUを含むportable index定義、再開可能なTitan再Embedding・bulk投入コマンドを実装した。Serverless対応APIに合わせてindex health、GET multi-search、count収束待ちを使用する。実collectionでのAPI互換性と検索は未確認 | AgentCore Runtimeの実行roleからTLSとIAM SigV4（`aoss`）を使用してhealth、BM25、vector、multi-searchが確認でき、管理taskからkuromoji + ICU mappingのindex作成・日本語token解析・投入ができ、全vectorがTitan V2で再生成され、同じ日本語datasetで`bge-m3`との差が評価されている |
...
| `AWS-016` | P0 | 検証待ち | 公開済みローカルsnapshotをAWS初期データとして再利用する | schema v3成果物へindex定義、16,459 Content Unit、124 node / 172 edge、published Run、14 XML・6 PDFを固定し、27ファイルのhashをread-only実データで検証した。S3 upload、URI変換、Titan再Embedding、OpenSearch / Neptune冪等投入と件数検証を実装した。AWSへの実投入は未実施 | manifest hashを検証してS3から固定成果物を読み、Titan vectorを生成してkuromoji indexへ16,459件、Neptuneへ124 node / 172 edgeを投入し、各snapshot・Run・件数一致、全件検索とmini Graph範囲の疎通を確認できる |
