...
| `AWS-013` | P0 | 対応中 | AWS環境の差分と構成変更を安全に扱う | schema version 8の環境JSONとaccount / region / CIDR / AgentCore対応AZ ID検証を実AWSで使用した。環境全体の`ConfigurationHash`が全resource tagへ伝播し、bootstrap image tagだけの変更でもOpenSearch Serverless collection置換を要求することが判明した。data stackはrollback済みでresourceを保持し、management / runtimeは`--exclusively`で更新している。stack別fingerprintへの移行とdata resourceのblue/green手順は未実装 | 再利用IaC、環境別設定、deploy outputが分離され、別設定のplan検証、resource置換時のデータ移行・疎通確認・rollbackを追跡できる |
