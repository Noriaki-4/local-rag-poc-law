...
| `AWS-005` | P0 | 検証待ち | Neo4j依存をNeptune Analytics接続へ分離する | Neptune Analytics LPGへ固定snapshotの124 node / 172 edgeを冪等投入し、VPC内read-only taskから件数、全relation種別、published Run 17/17・24 Assertion、公開買付府令9条→施行令9条の3→金商法27条の3の2ホップを直接確認した。openCypher非対応のlist propertyはAWS投入adapterでversion付きJSON文字列へ変換し、Runtime adapterで元のlist契約へ復元する。AgentCoreのGraph tool単独traceは未確認 | 現行Graph契約を変えずにNeptune Analytics用adapterからLPG投入、1ホップ探索、RelationAssertion探索、再投入ができる |
