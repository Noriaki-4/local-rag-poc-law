...
repository_name="$(config_value bootstrapData.imageRepositoryName)"
image_tag="$(config_value bootstrapData.imageTag)"
...
docker buildx build \
  ...
  --file "${repository_root}/infra/aws/bootstrap/Dockerfile" \
  --tag "${image_uri}" \
  --push \
  "${repository_root}"
