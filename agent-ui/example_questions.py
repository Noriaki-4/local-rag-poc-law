...
質問文には参照先の法令名・資料名を書かない。法令名を書くと検索対象を人が指定したことになり、
「必要な資料を自力で見つけられるか」を試せなくなるため。expected、required_evidence、
answer_points は答え合わせ専用で、質問文やAgent APIへのリクエストには含めない
...
EXAMPLE_QUESTIONS: tuple[ExampleQuestion, ...] = (
    ...
    ExampleQuestion(
        ...
        answer_points=(
            ...
            _point("過去3か月の勧誘人数との合算", ("3か月", "合算"), ("三か月", "合算")),
    ...
    ExampleQuestion(
        ...
        question=(
            "上場会社の株式を、市場を通さずに多数の株主から買い集めたいのですが、"
            "公開買付けの手続が必要になるのはどのような場合ですか。対象となる株券等の範囲、"
            "主な例外、必要な手続も含めて、根拠となる条文とともに説明してください。"
        ),
