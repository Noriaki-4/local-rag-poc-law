...
| `LR-020` | P1 | 対応中 | 検索又は結論を変える質問の欠落・曖昧さを一方的に補わず、利用者へ確認する | 検索前の独立した質問確認を実装した。`QuestionReadiness`契約で`ready / clarification_required`を判定し、一回の検索を一つの主体・行為ペアに限定する。必要時だけStreamlitへ確認質問、2〜4件の修正案及び直接修正欄を表示し、利用者が修正版を確認するまで`/answer`を実行しない。質問確認は検索結果、WorkItem、Hypothesis及びCaseStoreを入力しない | 再実行可能な評価fixtureを追加してモデル差を継続評価する。検索後の分岐で必要となるCaseの確認待ち・再開は次段階として設計する |
...
| `LR-022` | P0 | 実装済み | 後続CycleでHypothesisの現在版をブラッシュアップする | 同じ法的命題の見立てを修正する場合は同じHypothesis IDの現在版を更新し、旧版はCaseStoreの履歴へ保存する。通常のAgentViewには現在版だけを投影する。既存命題とは独立した確認事項が判明した場合だけ新IDを追加する。見直し入力は当Cycleで反証されたHypothesisと対応本文だけに限定する | 更新・履歴保存・現在版だけの投影は回帰済み。実モデルの少人数私募では不要な見直し呼出しが0回となり6/12から11/12へ回復。残る金商法23条の13の探索漏れはLR-042で扱う |
