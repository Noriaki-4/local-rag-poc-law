...
| `LR-023` | P0 | 回帰・再調査中（Profile v434） | Hypothesisに合うGraph方向と候補の対応先を安定して選ぶ | Profile v425では、具体化規定を探す4件の`IMPLEMENTS`要求が全て`to_subject`となった。Profile v426では、各意味関係のSUBJECT / OBJECT役割と方向の対応を共通Tool PromptとTool schemaへ直接記載し、4要求全てが修復なしで`IMPLEMENTS / from_subject`となった。しかしProfile v434の総合E2Eでは、同じ下位具体化目的のうち金商法27条の2、施行令7条、金商法27条の3を起点とする要求が再び`to_subject`となった | v434では施行令7条から既取得の上位規定27条の2へ戻り、府令2条の5を取得できず10/11となった。既取得本文の再統合と代替探索も増え、速度低下にも影響した。物理`REFERENCES`の探索目的変換とGraph候補のHypothesis再対応付けは別契約として維持し、結合後Prompt・入力・生出力で方向説明が埋もれていないかを隔離確認する |
...
`ADJACENT_INTEGRATION_WITHOUT_NEW_TOOL_RESULT`として誤検出した。直前と現在のHypothesis scopeが
重なる場合だけ同警告を出すよう修正し、保存済み診断の再集計で実行経路上の警告が0件になることを確認した。
