...
| `LR-013` | P0 | Haiku輸送追随修正済み／実モデル検証待ち | Provider共通の小さいSolver輸送契約へ統一する | v154で全Providerを同じ処理段階別schemaへ統一し、実行時IDはenumへ複製せず共通validatorで検証する。Luna基準へ切り替えた後、WorkItem別取得枠を5件へ変更してもAnthropic専用fetch sidecarだけ旧4件上限と取得枠0のWorkItem候補を保持していた。また、Haiku 4.5は`effort`非対応である一方、manual extended thinkingの送信経路がなかった | Anthropic fetch sidecarを共通Tool上限とWorkItem別残数から生成し、取得枠0のWorkItemを除外する。Haiku 4.5では設定可能なmanual extended thinkingをProvider adapter内だけで有効化し、共通のSolver、Cycle、Graph、検証契約は変更しない。Provider回帰後、同じcheckpointをHaikuで再生し、Integrationの400エラー、契約修復回数、取得Article数を確認する |
...
| `LR-047` | P0 | 構造・Prompt修正済み／速度改善継続（Profile v489） | 回答に必要な範囲を超えて未確認事項を広げ、本文評価と探索を繰り返す | LawQA 2問目に加え、Profile v488の裁量選択では方法別条件のWorkItemだけで本文統合10回・231.6秒、総合では本文統合7回・307.3秒を要した。いずれも必要な回答要素と条文は揃ったが、付随する例外、準用先又は詳細手続を新しい`gaps`へ加え、回答範囲を超えて追跡した。総合は同一検索の重複とWorkItem timeoutも各1件発生し、567.7秒となった。候補評価待ちのWorkItemが一つあると別WorkItemのTool要求まで保留する境界バグと、評価後の`fetchable_article_ids`へ古い検索抜粋のArticle IDが再混入する投影バグも反復を増やしていた。v489の少人数私募では、残った3組の重複本文取得がすべて別の`search-review-fetch`要求同士だった。後続の検索候補評価が、同じWorkItemで取得済みのArticleを再選択し、自動本文取得を再実行していた | Profile v489では保留を該当WorkItemだけへ限定し、取得可能IDをWorkItem別の取得状態を反映した評価済み候補から再構成する。gap追加前に回答範囲を確定し、その内容が未確認だと必要な限定付き回答ができない場合だけ追加するようPromptと自己点検を整理した。後続修正では、Evidence Integration専用輸送とSearch Review後の自動本文取得へWorkItem別・Case全体の既取得除外を共通適用した。同じWorkItemの再取得は状態遷移境界でも実行前に棄却し、別WorkItemの独立取得とCycleごとの取得枠は維持する。APIを使わない全1187テストに合格。最新の公告・例外E2Eでは同一WorkItem・Articleの重複取得0件で満点を確認した。回答範囲と速度の改善は継続する |
...
| `LR-050` | P0 | 完了（Profile v489） | 終了済みWorkItemの保留Graph候補を最終化時にLLMへ全件再判定させる | Profile v483では全WorkItemとHypothesisの終了後も、保留Graph候補4件を`deferred_frontier_resolutions`へ全件列挙する契約を最終化へ課した。Lunaは同じ全件性違反の修復を6回繰り返し、401.8秒で`protocol_error`となった。v489の総合では、open WorkItemの保留候補2件をCycle Close出力から漏らし、同じ全件性違反を3回繰り返した | 終了済みWorkItemの保留候補はProgramが`no_longer_needed`へ機械更新する。open WorkItemの候補はLLMの明示判断を保持し、Cycle Closeで列挙が漏れた候補だけをProgramが`carry_forward`へ補完する。通常stepで返した境界専用の処理結果はCycle Closeまで適用しない。候補の法的関連性はProgramで判断しない。v489の総合11/11、例外10/10でCycle境界を正常通過した |
