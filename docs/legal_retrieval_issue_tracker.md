...
| `LR-035` | P1 | 未着手 | LLMへ送る指示・入力・出力契約を、1つのレビュー用成果物で確認できるようにする | `snapshot`診断は`request.txt`へ固定指示と動的入力を保存するが、Providerへ別指定する出力schemaは`output_schema.json`に分かれている。診断JSONLにも両方は記録されるものの、LLM呼出しの契約全体を1ファイルで読める成果物はない。複数ファイルの照合が必要なため、Promptと契約の不整合をレビューしにくい | 同じ`RenderedModelCall`から、実送信テキスト、Provider出力schema、model・Provider・hashを区別して収録した`complete_request.md`又は`complete_request.json`を生成する。実際には別送信である項目を単一Promptと誤表示せず、既存成果物とのhash一致をテストする。診断出力だけを追加し、LLM入力とtoken数は変えない |
| `LR-036` | P1 | 実装中（時間境界を追加） | 各WorkItemを専属セッションへ固定し、異なるWorkItemだけを並列処理する | 現行Evidence IntegrationはWorkItem別に投影して最大4呼出しを並列化するが、呼出しは匿名であり、後続stepが同じ処理系列だとtraceと完成Promptから識別できない。さらに`executor.map`の全件待ちにより、1件の遅延・timeoutで完了済みの兄弟WorkItem差分まで失う可能性がある。Providerの会話IDを正本にすると、モデル切替、再開及びCaseStore永続化もProvider状態へ依存する | `case_id + work_item_id`から安定した論理session IDを作り、同じWorkItemのturnを直列化する。親AgentLoopが並列batchの共通締切とCycle・Runの終了を管理し、締切までの完了分だけを入力順でCaseStoreへ反映する。未完了WorkItemはopenのまま残し、子sessionはCycle Close、最終回答及びCaseStore更新を行わない。fixtureでsession固定、turn継続、scope分離、部分timeout時の完了差分保持及び決定的mergeを確認し、Luna `high`総合問題で11/11と所要時間を比較する |
