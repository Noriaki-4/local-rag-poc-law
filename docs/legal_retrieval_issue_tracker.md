...
| `LR-022` | P0 | 実装済み | 後続CycleでHypothesisの現在版をブラッシュアップする | 同じ法的命題の見立てを修正する場合は同じHypothesis IDの現在版を更新し、旧版はCaseStoreの履歴へ保存する。通常のAgentViewには現在版だけを投影する。既存命題とは独立した確認事項が判明した場合だけ新IDを追加する。見直し入力は当Cycleで反証されたHypothesisと対応本文だけに限定する | 更新・履歴保存・現在版だけの投影に加え、旧版に依存する派生判断とToolRequestの失効を回帰済み。更新後の同一Cycle内Requestは現在版の実績として扱う。実モデルの少人数私募では不要な見直し呼出しが0回となり6/12から11/12へ回復。残る金商法23条の13の探索漏れはLR-042で扱う |
...
| `LR-024` | P0 | 再発修正済み・構造化継続検討（Profile v480） | Hypothesisが支持された内容と未確認事項を同時に保持する | Haiku・Profile v375では、H-3は府令2条の5、H-4は府令10条を未取得だった。それぞれ上位規定から一部内容を確認できたが、`judgment=supported`への更新と同時に`gaps=[]`となった。Profile v479では別の未確認事項を追加した際、既存の「府令が定める電子公告の具体的方法」を配列から置換して失い、公開買付府令9条を保留したままWorkItemを閉じた | Profile v480ではEvidence Integrationの手順を、既存gapごとの照合、今回の本文で確認できた項目だけの除去、新規gapの追記という順にした。非居住公開買付者E2Eは府令9条を取得して12/12に合格した。文字列配列全体の置換が再び不安定になる場合は、`gap_id`付き項目と`add_gaps / resolve_gap_ids`の差分契約へ移行する |
