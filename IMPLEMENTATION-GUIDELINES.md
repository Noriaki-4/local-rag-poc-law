...
- 応答時間を短縮するため、互いに独立した処理は並列実行を優先します。並列化の前に、処理間の順序依存、共有状態の更新及び上限値の適用単位を確認します。
- 異なるWorkItemを並列化する場合も、複数WorkItemのTool要求や意味評価を1つのLLM処理へまとめません。各結果は所属WorkItemを保ってCaseStoreへ適用します。
...
- 命題の意味が変わるときは既存状態を上書きせず、置換元を明示した新しいHypothesisとして保存します。
- 置換前のHypothesisは監査用に保持し、通常のLLM入力には置換されていないHypothesisだけを渡します。
- Evidenceとの対応は差分追加し、既存の対応を失わないようにします。
- 未確認事項は追加・破棄・解消の差分で更新し、配列全体の再生成によって既存項目を失わないようにします。本文で確認できた項目は解消、不適切又は重複した項目は破棄、内容の訂正は旧項目の破棄と新項目の追加で表します。
