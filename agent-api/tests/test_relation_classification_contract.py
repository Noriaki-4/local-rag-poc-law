...
def test_candidate_key_changes_with_snapshot_model_or_content() -> None:
    ...
    changed_subject = candidate.subject.model_copy(update={"content_hash": "changed"})
    assert candidate.model_copy(update={"subject": changed_subject}).candidate_key != candidate.candidate_key
    ...
    decision = RelationClassificationDecision(
        ...
        assertions=(
            ProposedRelationAssertion(
                ...
                subject_supporting_span_id=candidate.subject.spans[0].span_id,
                object_supporting_span_id=candidate.object.spans[0].span_id,
            ),
        ),
    )
    ...
    decision = RelationClassificationDecision(
        ...
        assertions=(
            ProposedRelationAssertion(
                proposed_predicate=ProposedPredicate.IMPLEMENTS,
                reference_occurrence_hash="occ-1",
                subject_supporting_span_id=candidate.object.spans[0].span_id,
                object_supporting_span_id=candidate.subject.spans[0].span_id,
            ),
        ),
    )
...
def test_build_assertion_records_copies_only_validated_llm_choices() -> None:
    ...
    assert records[0].subject_supporting_quote == candidate.subject.spans[0].text
    assert records[0].object_supporting_quote == candidate.object.spans[0].text
