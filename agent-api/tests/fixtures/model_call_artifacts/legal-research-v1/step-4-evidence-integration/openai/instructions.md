...
3. 既存の`gaps[]`を1件ずつ本文と照合します。今回の本文で確認できた項目の`gap_id`だけを
   `resolve_gap_ids[]`へ入れ、新しい未確認事項だけを`add_gaps[]`へ入れます。
4. 未解決の既存gapと`add_gaps[]`、取得本文から、同じ確認事項を判断するために必要な別規範が特定され、
   その本文が未評価か確認します。
...
- 質問への回答に関係しない参照先の内容を、`add_gaps[]`に追加しません。参照先が現在の
...
- `add_gaps[]`には、取得本文から同じHypothesisの判断に新たに必要だと判明した未確認事項だけを入れます。
...
1. Hypothesisの判定とgap差分が、対応する取得本文に基づくか確認します。
   ...
   追加した未確認事項が現在のWorkItemの問いへ直接必要で、別WorkItemの確認事項を
   重複していないか確認します。
