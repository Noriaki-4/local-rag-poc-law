...
3. `statement`全体を判定できなくても、本文で確認できた`gaps`の部分はEvidenceを対応付け、残る未確認内容だけを`gaps`へ残します。
4. Hypothesisの根拠に使う各文について、「別規範で定める」旨の委任表現を確認します。
   WorkItemで問う事項を委任していれば、その委任先本文が揃ったか判断します。
...
- 下位規範を追うToolは、取得本文がWorkItemの未確認事項を別規範へ明示的に委任している場合だけ返します。
  Toolの`purpose`には、本文中の委任と委任事項を書きます。「特則又は詳細がある可能性」は理由になりません。
...
- 質問への回答に関係しない参照先の内容を、新しい`gaps`に追加しません。取得本文が質問された
  条件、範囲又は手続を別規範へ明示的に委任している場合だけ、その委任先本文を確認対象に残します。
...
- WorkItemが求める既存の`gaps`は、対応する内容を本文で確認した場合だけ削除します。WorkItemが求めず、
  取得本文も別規範へ委任していない詳細は、次の探索理由にせず確認範囲から外します。
- 更新後の`gaps`は、既存の未確認事項の残りと、取得本文が同じ事項として明示的に委任した内容だけです。
...
- 取得本文が確認事項を別規範へ明示的に委任している場合は、その委任先本文を確認するまで
  `terminal_text_missing`です。明示的な委任がなければ、予想した詳細だけを理由にこの状態にしません。
...
- `terminal_text_missing`のWorkItemでは、本文が明示的に委任した未確認内容を、対応するHypothesisの
  少なくとも1件の`gaps`に残します。
