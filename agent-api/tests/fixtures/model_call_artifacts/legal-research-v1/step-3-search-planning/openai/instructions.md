...
- WorkItemに明示された確認事項が、より広い検索語へ置き換えられて消えていない。

## 手順

1. WorkItemの事実を検索範囲、Hypothesisの法律関係と法的効果を確認対象、`gaps`を未確認事項として読み分けます。任意の`answer_options[]`は未確認の回答候補として参照します。
2. 各Hypothesisについて、質問中の行為と、その行為に伴って変動する権利又は契約関係を区別します。
3. `available_tools`で`legal_search`の用途、入力項目、戻り値を確認します。
4. 今回の上限内で検索要求を作ります。`gaps`があればその内容を、空なら未検証の`statement`を検索語の中心にし、手順2で確認した法律関係と法的効果を含めます。
5. 完了条件を確認します。
...
- Hypothesis又は`gaps`がWorkItemより広い表現でも、WorkItemに明示された条件、効果又は手続を検索対象から落としません。
- 1つの検索要求は、同じWorkItemに属する1件以上のHypothesisを対象にします。
- 各Hypothesis IDは、今回の`search_requests`のうち1件だけに含めます。複数の観点がある場合は、値補を発見できる法令用語を1つの`query`に組み合わせます。
...
- 日常的な行為の説明から法律関係を読み取れる場合は、行為の名称だけで終えず、その行為に伴って発生、移転、変更又は消滅する権利若しくは契約関係を表す法令用語も検索語に使います。検索語は候補発見の手掛かりであり、法的結論の確定ではありません。
- 契約の対象物を処分する場面では、対象物の処分だけでなく、それに伴う権利又は契約上の地位の移転も検索対象として検討します。
- 用途によって適用規律を比較する場面では、各用途に対応する法律関係を区別できる法令用語を使います。
- 一般規定と特則、又は異なる適用範囲を比較するWorkItemでは、一方だけに限定せず、それぞれの法律関係を識別できる語を含めます。
- 法令本文に現れにくい業務上の呼称は、既知の主体属性と、義務又は権限を表す法令表現へ言い換えます。
- 数値、期間又は定義を確認する場合は、広い制度名や提出書類名を重ねず、その基準を定める条文に現れやすい特徴的な表現を優先します。
- 検索語には、確認する規律の法的効果、対象、関係又は手続を表し、条文や見出しに現れやすい表現を使います。
- 委任を確認する場合は、WorkItem、Hypothesis及び`gaps`から読み取れる委任事項と委任先を検索語に反映します。
...
- `action_feedback`がある場合は、そこに示された未実行理由に合わせて検索要求を組み直します。
