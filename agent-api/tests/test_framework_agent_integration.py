...
from app.adapters.models.structured_json import (
    ...
    _cycle_close_deferred_frontiers,
    ...
    _normalize_cycle_close_decisions,
    ...
    _search_review_work_item_contexts,
    ...
    _validate_hypothesis_update_evidence,
...
from app.agent_framework.loop import (
    AgentLoop,
    _current_cycle_exploration_set_complete,
    _dependency_audit_scope,
    _dependency_audit_work_item_ids,
    _grounding_observation_context,
    _has_pending_known_candidate_processing,
    _project_completed_exploration_set_to_cycle_close,
)
...
from app.agent_framework.ports.tool import ToolDefinition, ToolRegistry
...
def test_integration_projection_omits_duplicate_evidence_metadata() -> None:
    fixture_path = (
        Path(__file__).parent
        / "fixtures"
        / "framework"
    ...
    payload = _solver_context_payload(context, projection="integration")

    assert "evidence_manifest" not in payload
    assert "navigation_evidence_ids" not in payload
    assert payload["material_evidence"]
    assert all(
        set(item) == {"evidence_id", "content", "title"}
        for item in payload["material_evidence"]
    )
    assert set(payload["grounding_evidence_ids"]) == {
...
def test_new_framework_uses_legal_tool_and_skips_reviewer_by_default(
    ...
    assert "新たに判明した未確認事項" in llm.calls[4]["schema"]["properties"][
        "update_hypotheses"
    ]["items"]["properties"]["add_gaps"]["description"]
    ...
    assert diagnostic_records[0]["profileVersion"] == "510"
    ...
    assert transport_input["profileVersion"] == "510"
    assert transport_input["modelCallStage"] == "solver_research_decomposition"
    assert transport_input["promptBuilder"].endswith(":_solver_prompt")
    assert transport_input["promptAssets"] == []
    assert transport_input["effectiveReasoningMode"] == "provider_default"
    assert transport_input["effectiveReasoningEffort"] is None
    assert transport_input["thinkingBudgetTokens"] is None
...
def test_framework_snapshot_diagnostics_preserve_full_solver_material(
    ...
    hypotheses = {
        item["hypothesisId"]: item for item in record["stateStatus"]["hypotheses"]
    }
    assert hypotheses["h-1"]["active"] is False
    assert hypotheses["h-2"]["active"] is True
    assert hypotheses["h-2"]["replacesHypothesisId"] == "h-1"
...
def test_dependency_audit_scope_keeps_other_pending_work_with_fresh_observation(
) -> None:
    state = CaseState(
        case_id="case-1",
        question="質問",
        work_items=(
            WorkItem(work_item_id="w1", question="例外"),
            WorkItem(work_item_id="w2", question="手続"),
        ),
        hypotheses=(
            Hypothesis(hypothesis_id="h1", work_item_id="w1", statement="例外"),
            Hypothesis(hypothesis_id="h2", work_item_id="w2", statement="手続"),
        ),
        tool_requests=(
            ToolRequest(
                request_id="fetch-1",
...
def test_pending_dependency_action_context_keeps_other_work_item_eligible(
) -> None:
    state = CaseState(
        case_id="case-1",
        question="質問",
        ...
        retained_evidence_ids=("body-1", "body-2"),
        dependency_decisions=(
            DependencyDecision(
                dependency_kind="lower_norm",
                work_item_id="w2",
    ...
    projected = _pending_dependency_action_contexts(context)

    assert len(projected) == 1
    assert projected[0].required_dependency_work_item_ids == ("w2",)
    assert projected[0].remaining_fetch_capacity == (
        context.remaining_fetch_capacity_by_work_item["w2"]
    )


def test_observation_step_advances_other_pending_work_item_in_parallel() -> None:
    state = CaseState(
        case_id="case-1",
        question="質問",
        research_cycle_count=2,
    ...
    class ObservationAndPendingActionLLM:
        ...
        def generate_structured_json(self, **kwargs: Any) -> StructuredJSONResult:
            ...
            if set(properties) == {"decision_reason", "dependency_decisions"}:
                ...
                payload = {
                    "decision_reason": "手続の探索方針を次Cycleで見直す。",
    ...
    assert {item.work_item_id for item in result.decision.dependency_decisions} == {
        "w1",
        "w2",
    }
...
def test_observation_profile_precedes_pending_search_review(
    ...
    context = build_solver_context(
        ...
        update={"required_search_review_request_ids": ("search-1",)}
    )
    ...
    monkeypatch.setattr(
        adapter,
        "_solve_observation_only",
        lambda _context, _profile: expected,
    )
    monkeypatch.setattr(
        adapter,
        "_solve_search_review",
        lambda _context, _profile: pytest.fail(
            "observation must run before pending candidate review"
        ),
    )
    profile = legal_profiles.legal_agent_profile().solver_observation_integration
    assert profile is not None

    assert adapter.solve(context, profile) is expected
...
def test_legal_solver_prompts_are_projected_by_structural_mode() -> None:
    ...
    mode_prompts = {
        ...
        "hypothesis_revision": (
            profile.solver_hypothesis_revision.system_prompt
        ),
    ...
    completion_checks = {
        ...
        "hypothesis_revision": (
            profile.solver_hypothesis_revision.completion_check_prompt
        ),
    ...
    assert profile.solver_hypothesis_revision.context_projection == (
        "hypothesis_revision"
    )
    assert profile.solver_integration.context_projection == "integration"
    ...
    assert "質問にない行為者又は相手方の類型へ置き換えません" in (
        search_prompt
    )
    assert "WorkItemにない判定軸" in search_prompt
    assert "Hypothesisまたは`gaps`を検証" not in search_prompt
    assert (
        profile.limits.max_graph_articles_per_hypothesis_per_cycle == 2
    )
    ...
    assert "取得済み本文と未評価候補を先に処理します" in integration_prompt
    ...
    assert "現在セットで未使用のOpenSearch又はGraph" in integration_prompt
    ...
    assert "# 法令調査Solver：Cycleの引継ぎ" in transition_prompt
    assert "最終回答は別の処理が作ります" in transition_prompt
    assert "取得本文の再評価、次の検索計画、最終回答を行っていない" in (
        transition_prompt
    )
    ...
    finalization_prompt = mode_prompts["finalization"]
    assert "## 最終回答" in finalization_prompt
    assert "調査終了時" in finalization_prompt
...
def test_hypothesis_generation_runs_work_items_in_parallel() -> None:
    class ParallelHypothesisClient:
        provider = "fake"
        ...
        def generate_structured_json(self, **kwargs: Any) -> StructuredJSONResult:
            work_item_id = kwargs["schema"]["properties"]["hypotheses"][
                "items"
            ]["properties"]["work_item_id"]["enum"][0]
            with self._lock:
                self._active += 1
                    ...
                    "hypotheses": [
                        {
                            ...
                            "statement": f"{work_item_id}の命題",
                            "gaps": [f"{work_item_id}の未確認事項"],
                        }
                    ]
                },
                provider="fake",
    ...
    state = CaseState(
        case_id="case-parallel-hypotheses",
        question="2つの論点を確認する。",
        work_items=(
            WorkItem(work_item_id="wi-1", question="論点1"),
            WorkItem(work_item_id="wi-2", question="論点2"),
        ),
    )
    context = build_solver_context(
        state,
        AgentLimits(max_parallel_work_items=2),
        remaining_wall_time_sec=120,
        finalize_only=False,
    )
    client = ParallelHypothesisClient()
    adapter = StructuredJSONModelAdapter(client)  # type: ignore[arg-type]
    result = adapter.solve(
        context,
        ModelCallProfile(
            model="fake",
            system_prompt="WorkItemごとにHypothesisを作る。",
    ...
    assert [
        (item.hypothesis_id, item.work_item_id)
        for item in result.decision.update.add_hypotheses
    ] == [("h-1", "wi-1"), ("h-2", "wi-2")]
...
def test_search_planning_runs_work_items_in_parallel() -> None:
    class ParallelSearchPlanningClient:
        provider = "fake"
        ...
        def generate_structured_json(self, **kwargs: Any) -> StructuredJSONResult:
            request_schema = kwargs["schema"]["properties"]["search_requests"][
                "items"
            ]["properties"]
            work_item_id = request_schema["work_item_id"]["enum"][0]
            hypothesis_id = request_schema["hypothesis_ids"]["items"]["enum"][0]
            with self._lock:
                self._active += 1
                self.max_active = max(self.max_active, self._active)
            sleep(0.05)
                    ...
                    "search_requests": [
                        {
                            ...
                            "purpose": f"{work_item_id}を確認する",
                            "query": f"{work_item_id} 法令用語",
                            "doc_types": ["law"],
                        }
                    ]
                },
    ...
    state = CaseState(
        case_id="case-parallel-search-planning",
        question="2つの論点を検索する。",
        work_items=(
            WorkItem(work_item_id="wi-1", question="論点1"),
            WorkItem(work_item_id="wi-2", question="論点2"),
    ...
    client = ParallelSearchPlanningClient()
    result = StructuredJSONModelAdapter(client).solve(  # type: ignore[arg-type]
        context,
        ModelCallProfile(
            model="fake",
            system_prompt="WorkItemごとに検索要求を作る。",
    ...
    assert [
        ...
        for item in result.decision.tool_requests
    ] == [("wi-1", ("h-1",)), ("wi-2", ("h-2",))]
    assert result.input_tokens == 20
    assert result.output_tokens == 10
    assert result.attempt_count == 2
    ...
    assert len(applied.tool_requests) == 2


def test_search_selection_runs_work_items_in_parallel() -> None:
    class ParallelSearchSelectionClient:
        provider = "fake"
        ...
        def generate_structured_json(self, **kwargs: Any) -> StructuredJSONResult:
            selection_schema = kwargs["schema"]["properties"]["selections"][
                "items"
            ]["properties"]
            article_id = selection_schema["article_id"]["enum"][0]
            hypothesis_id = selection_schema["matched_hypothesis_ids"]["items"][
                "enum"
            ][0]
            with self._lock:
                self._active += 1
                    ...
                    "selections": [
                        {
                            "article_id": article_id,
                            "legal_function": "applicability",
                            "summary": f"{article_id}の規律",
                            "matched_hypothesis_ids": [hypothesis_id],
                            "reason": f"{hypothesis_id}を確認できる",
                        }
                    ],
                    "reason": "本文取得対象を選んだ",
                },
    ...
    requests = (
        ToolRequest(
            ...
            work_item_id="wi-1",
            tool_name="legal_search",
            arguments={"query": "論点1", "doc_types": ["law"]},
            purpose="論点1を探す",
            hypothesis_ids=("h-1",),
        ),
        ToolRequest(
            ...
            work_item_id="wi-2",
            tool_name="legal_search",
            arguments={"query": "論点2", "doc_types": ["law"]},
            purpose="論点2を探す",
            hypothesis_ids=("h-2",),
        ),
    )
    ...
    state = CaseState(
        case_id="case-parallel-search-selection",
        question="2つの候補を選ぶ。",
        research_cycle_count=1,
        work_items=(
            WorkItem(work_item_id="wi-1", question="論点1"),
        ...
        update={
            ...
            "search_candidates": (
                ...
                SearchCandidateArticle(
                    ...
                    discovery_hypothesis_ids=("h-2",),
                    search_request_ids=("search-2",),
                    navigation_evidence_ids=("nav-2",),
                ),
            ),
            "navigation_evidence_ids": ("nav-1", "nav-2"),
            "material_evidence": evidence,
            "fetchable_article_ids": ("article-1", "article-2"),
            "required_search_review_request_ids": ("search-1", "search-2"),
            "remaining_fetch_capacity": 10,
            "remaining_fetch_capacity_by_work_item": {"wi-1": 5, "wi-2": 5},
        }
    )
    client = ParallelSearchSelectionClient()
    result = StructuredJSONModelAdapter(client).solve(  # type: ignore[arg-type]
        context,
        ModelCallProfile(
            model="fake",
            system_prompt="WorkItemごとに候補を選ぶ。",
            timeout_sec=2,
        ),
    )

    review = result.decision.search_candidate_review
    assert review is not None
    assert client.max_active == 2
    assert review.search_request_ids == ("search-1", "search-2")
    assert [item.article_id for item in review.selections] == [
        "article-1",
        "article-2",
    ]
    assert [item.matched_hypothesis_ids for item in review.selections] == [
        ("h-1",),
        ("h-2",),
    ]
    assert review.deferred_article_ids == ()
    assert result.input_tokens == 20
    assert result.output_tokens == 10
    assert result.attempt_count == 2
    applied = apply_solver_decision(
        state,
        result.decision,
        limits=AgentLimits(max_parallel_work_items=2),
    ...
    assert len(applied.search_candidate_reviews) == 1
...
def test_search_reselection_uses_work_item_fetch_capacity() -> None:
    fixture_path = (
        Path(__file__).parent
        / "fixtures"
        / "framework"
    ...
    context = saved.model_copy(
        update={
            ...
            "remaining_fetch_capacity_by_work_item": {
                item.work_item_id: 5 for item in saved.work_tree
            },
    ...
    assert rendered.input_payload["remaining_fetch_capacity_by_work_item"]
    assert rendered.output_schema["properties"]["selections"]["maxItems"] == 5
    assert "remaining_fetch_capacity_by_work_item" in rendered.instructions
...
def test_cycle_close_does_not_retain_evidence_already_managed_by_state() -> None:
    ...
    assert "自動再提示される" in rendered.instructions
...
def test_cycle_close_tells_model_the_retained_evidence_limit() -> None:
    ...
    assert "必要に応じて引き継ぎます" in rendered.instructions
...
def test_cycle2_anthropic_solver_uses_provider_transport_schema() -> None:
    ...
    assert len(json.dumps(rendered.output_schema)) < 2_000
...
def test_legal_profile_applies_model_levels_per_llm_purpose(monkeypatch) -> None:
    monkeypatch.setattr(
        legal_profiles.settings,
...
def test_search_planning_preserves_legal_relationship_and_explicit_issues() -> None:
    search_prompt = legal_profiles.legal_agent_profile().solver_search_planning
    assert search_prompt is not None
    prompt = search_prompt.system_prompt

    assert "変動する権利又は契約関係" in prompt
    assert "WorkItemに明示された確認事項" in prompt
    assert "より広い検索語へ置き換えられて消えていない" in prompt
    assert "各Hypothesis ID" in prompt
    assert "1件だけに含めます" in prompt
    assert "`action_feedback`がある場合" in prompt


def test_search_planning_projects_rejected_action_feedback() -> None:
    state = CaseState(
        case_id="case-search-action-feedback",
        question="確認事項を調べる。",
        work_items=(WorkItem(work_item_id="wi-1", question="確認事項"),),
        hypotheses=(
            Hypothesis(
    ...
    context = build_solver_context(
        ...
        update={
            "action_feedback": SolverActionFeedback(
                ...
                message=(
                    "the same exploration tool was already used for this "
                    "Hypothesis in the current Cycle"
                ),
    ...
    payload = _solver_context_payload(context, projection="research_search")

    assert payload["action_feedback"]["code"] == "already_completed"
    assert payload["action_feedback"]["rejected_tool_requests"][0][
        "request_id"
    ] == "rejected-1"
...
def test_supported_hypothesis_with_gaps_remains_available_for_search() -> None:
    ...
    assert matched_schema["minItems"] == 1
...
def test_contradicted_hypothesis_with_a_gap_keeps_pending_search_scope() -> None:
    request = ToolRequest(
        request_id="search-1",
        work_item_id="w1",
        tool_name="legal_search",
        ...
        purpose="承諾要件の直接規定を確認する",
        hypothesis_ids=("h-1",),
    )
    state = CaseState(
        case_id="case-contradicted-gap-search",
        question="借地権の譲渡に地主の承諾が必要か。",
        research_cycle_count=2,
        work_items=(
            WorkItem(
                work_item_id="w1",
        ...
        hypotheses=(
            Hypothesis(
                hypothesis_id="h-1",
                work_item_id="w1",
                statement="借地権の譲渡には地主の承諾が必要である。",
                judgment="contradicted",
                evidence_ids=("e1",),
                gaps=("賃借権譲渡の承諾要件を直接定める規定",),
            ),
        ),
        evidence=(
            Evidence(
    ...
    context = build_solver_context(
        ...
        update={
            "search_candidates": (
                SearchCandidateArticle(
                    article_id="law-civil-code-article-612",
                    document_id="law-civil-code",
                    title="民法",
                    headings=("第六百十二条",),
                    discovery_work_item_ids=("w1",),
                    discovery_hypothesis_ids=("h-1",),
            ...
            "recent_tool_requests": (request,),
            "remaining_fetch_capacity": 1,
            "remaining_fetch_capacity_by_work_item": {"w1": 1},
        }
    )

    projected = _search_review_work_item_contexts(context)

    assert len(projected) == 1
    assert [item.work_item_id for item in projected[0].work_tree] == ["w1"]
    assert [item.hypothesis_id for item in projected[0].hypotheses] == ["h-1"]
    rendered = render_search_selection_model_call(
        projected[0],
        legal_profiles.legal_agent_profile().solver_search_review,
    )
    assert rendered.input_payload["work_tree"][0]["work_item_id"] == "w1"
    assert rendered.input_payload["hypotheses"][0]["hypothesis_id"] == "h-1"
    matched_schema = rendered.output_schema["properties"]["selections"][
        "items"
    ]["properties"]["matched_hypothesis_ids"]
    assert matched_schema["items"]["enum"] == ["h-1"]

    in_flight_context = context.model_copy(
        update={
            "hypotheses": (
                context.hypotheses[0].model_copy(update={"gaps": ()}),
            )
        }
    )
    in_flight_projected = _search_review_work_item_contexts(in_flight_context)
    assert len(in_flight_projected) == 1
    assert [
        item.hypothesis_id for item in in_flight_projected[0].hypotheses
    ] == ["h-1"]
    in_flight_rendered = render_search_selection_model_call(
        in_flight_projected[0],
        legal_profiles.legal_agent_profile().solver_search_review,
    )
    assert [
        item["hypothesis_id"]
        for item in in_flight_rendered.input_payload["hypotheses"]
    ] == ["h-1"]


def test_search_selection_excludes_work_items_without_fetch_capacity() -> None:
    state = CaseState(
        case_id="case-search-capacity",
        question="確認事項",
        work_items=(
            WorkItem(work_item_id="w-full", question="取得枠を使用済み"),
            WorkItem(work_item_id="w-open", question="追加取得できる"),
        ),
        hypotheses=(
            Hypothesis(
                hypothesis_id="h-full",
                work_item_id="w-full",
                statement="取得枠を使用済みの命題",
            ),
            Hypothesis(
        ...
        update={
            "search_candidates": (
                SearchCandidateArticle(
                    ...
                    title="候補法令",
                    headings=("第一条",),
                    discovery_work_item_ids=("w-full", "w-open"),
                    discovery_hypothesis_ids=("h-full", "h-open"),
                    search_request_ids=("search-1",),
                    navigation_evidence_ids=(),
                ),
            ),
    ...
    assert matched_schema["items"]["enum"] == ["h-open"]
...
def test_dependency_action_fetch_schema_uses_each_work_item_capacity() -> None:
    fixture_path = (
        Path(__file__).parent
        / "fixtures"
        / "framework"
    ...
    work_item_ids = context.required_dependency_work_item_ids[:2]
    ...
    candidates = (
        SearchCandidateArticle(
            ...
            title=None,
            headings=(),
            discovery_work_item_ids=(work_item_ids[0],),
        ...
        SearchCandidateArticle(
            ...
            title=None,
            headings=(),
            discovery_work_item_ids=(work_item_ids[1],),
            discovery_hypothesis_ids=tuple(
    ...
    context = context.model_copy(
        update={
            "required_dependency_work_item_ids": work_item_ids,
            "available_tools": (LegalFetchArticlesTool.definition,),
            "search_candidates": candidates,
            "fetchable_article_ids": ("article-1", "article-2", "article-3"),
    ...
    limits_by_work_item = {
        item["properties"]["work_item_id"]["enum"][0]: item["properties"]
        ["arguments"]["properties"]["article_ids"]["maxItems"]
        for item in variants
    }
    assert limits_by_work_item == {
        work_item_ids[0]: 1,
        work_item_ids[1]: 2,
    }
    for item in variants:
        work_item_id = item["properties"]["work_item_id"]["enum"][0]
        assert set(
            item["properties"]["hypothesis_ids"]["items"]["enum"]
...
def test_anthropic_dependency_action_accepts_one_fetch_per_work_item() -> None:
    state = CaseState(
        case_id="case-anthropic-dependency-fetches",
        question="二つの下位規範を確認する。",
        research_cycle_count=1,
        work_items=(
            WorkItem(work_item_id="w1", question="一つ目を確認する。"),
            WorkItem(work_item_id="w2", question="二つ目を確認する。"),
        ),
        hypotheses=(
            Hypothesis(hypothesis_id="h1", work_item_id="w1", statement="命題1"),
            Hypothesis(hypothesis_id="h2", work_item_id="w2", statement="命題2"),
        ),
        dependency_decisions=(
            DependencyDecision(
                dependency_kind="lower_norm",
    ...
    context = build_solver_context(
        ...
        update={
            ...
            "remaining_fetch_capacity_by_work_item": {"w1": 1, "w2": 1},
        }
    )
    ...
    fetch_array = rendered.output_schema["properties"]["fetch_articles"][
        "anyOf"
    ][0]
    assert fetch_array["maxItems"] == 2

    decision = normalize_dependency_action_decision(
        {
            "decision_reason": "各WorkItemの既知候補を取得する。",
            "start_next_cycle": False,
            "tool_requests_json": "[]",
            "fetch_articles": [
                {"work_item_id": "w1", "article_ids": ["article-1"]},
                {"work_item_id": "w2", "article_ids": ["article-2"]},
            ],
        },
        context=context,
    )

    assert [item.work_item_id for item in decision.tool_requests] == ["w1", "w2"]
    assert [item.arguments["article_ids"] for item in decision.tool_requests] == [
        ["article-1"],
        ["article-2"],
    ]
...
def test_search_review_does_not_refetch_article_for_same_work_item() -> None:
    profile = legal_profiles.legal_agent_profile()
    loop = AgentLoop(
        store=InMemoryCaseStore(),
        model=StructuredJSONModelAdapter(FakeStructuredLLM()),
    ...
    state = CaseState(
        case_id="case-search-review-refetch",
        question="確認する。",
        research_cycle_count=1,
        work_items=(WorkItem(work_item_id="wi-1", question="確認事項"),),
        hypotheses=(
    ...
    requests = loop._search_review_fetch_requests(state, review, candidates)
...
def test_compact_transport_structures_update_and_tool_arguments() -> None:
    context = build_solver_context(
        ...
        available_tools=(
            ToolDefinition(
                name="legal_search",
                description="法令候補を検索する。",
                input_schema={
                    "type": "object",
                    "properties": {"query": {"type": "string"}},
                    "required": ["query"],
                },
                result_description="検索候補。",
            ),
        ),
    )
...
def test_anthropic_flattened_graph_request_is_restored_with_fetch_sidecar() -> None:
    normalized = _normalize_solver_payload(
        {
            "decision_json": json.dumps(
                {
                    "next": "continue",
                    "decision_reason": "明示参照と既知候補を確認する。",
                    "dependency_decisions": [],
                    "tool_requests": [
                        {
                            "type": "legal_graph_neighbors",
                            "request_id": "graph-req-1",
                            "work_item_id": "wi-1",
                            "hypothesis_ids": ["h-1"],
                            "article_ids": ["law-article-11"],
                            "mode": "reference_edges",
                            "predicate": None,
                            "reference_lookup": (
            ...
            "fetch_articles": {
                ...
                "work_item_id": "wi-1",
                "purpose": "既知候補の本文を確認する。",
                "hypothesis_ids": ["h-1"],
                "article_ids": ["law-article-33"],
            },
        }
    )

    graph_request = normalized["tool_requests"][0]
    ...
    assert normalized["tool_requests"][1]["tool_name"] == "fetch_articles"
    assert normalized["tool_requests"][1]["arguments"] == {
        "article_ids": ["law-article-33"]
    }


def test_contract_repair_limits_identical_graph_requests_to_one() -> None:
    ...
    context = build_solver_context(
        ...
        available_tools=(
            ToolDefinition(
                ...
                description="Graph候補を検索する。",
                input_schema={"type": "object"},
                result_description="Graph候補。",
            ),
        ),
    )
...
def test_solver_payload_consolidates_fetch_transport_without_dropping_links() -> None:
    normalized = _normalize_solver_payload(
        {
            ...
            "tool_requests": [
                ...
                },
                {
                    ...
                    "work_item_id": "w1",
                    "tool_name": "fetch_articles",
                    "arguments": {"article_ids": ["article-3"]},
                    "purpose": "第一の追加本文を取得する",
                    "hypothesis_ids": ["h3"],
                },
            ],
        }
    ...
    assert len(normalized["tool_requests"]) == 2
    first, second = normalized["tool_requests"]
    assert first["work_item_id"] == "w1"
    assert first["arguments"]["article_ids"] == ["article-1", "article-3"]
    assert first["hypothesis_ids"] == ["h1", "h3"]
    assert second["work_item_id"] == "w2"
    assert second["arguments"]["article_ids"] == ["article-2", "article-1"]
    assert second["hypothesis_ids"] == ["h2"]
    assert {
...
def test_solver_payload_keeps_identical_graph_transport_per_work_item() -> None:
    graph_arguments = {
        "article_ids": ["article-1"],
        "mode": "semantic_assertion",
        "predicate": "IMPLEMENTS",
    ...
    assert len(normalized["tool_requests"]) == 2
    assert [item["work_item_id"] for item in normalized["tool_requests"]] == [
        "w1",
        "w2",
    ]
    assert [item["hypothesis_ids"] for item in normalized["tool_requests"]] == [
        ["h1"],
        ["h2"],
    ]
    assert normalized["dependency_decisions"][0]["action_request_id"] == "graph-1"
    assert normalized["dependency_decisions"][1]["action_request_id"] is None
...
def test_diagnostic_integration_fixture_rebuilds_the_same_solver_context() -> None:
    ...
    assert rebuilt.model_dump(mode="json") == saved.model_copy(
        update={
            ...
            "remaining_fetch_capacity_by_work_item": (
                rebuilt.remaining_fetch_capacity_by_work_item
            ),
            "hypothesis_exploration_sets": rebuilt.hypothesis_exploration_sets,
        }
    ).model_dump(mode="json")
...
def test_real_model_initial_research_decomposition_fixture_is_reproducible() -> None:
    ...
    assert "候補となる法令種別が複数" in hypothesis_prompt.system_prompt
    ...
    assert rebuilt.model_dump(mode="json") == saved.model_copy(
        update={
            ...
            "completed_graph_searches": rebuilt.completed_graph_searches,
                ...
                "search_candidates": rebuilt.search_candidates,
                ...
                "fetched_resource_ids_by_work_item_this_cycle": (
                    rebuilt.fetched_resource_ids_by_work_item_this_cycle
                ),
...
def test_lr_003_second_hop_fixture_keeps_graph_replanning_actionable() -> None:
    ...
    with pytest.raises(
        ContractViolation,
        match="same exploration tool was already used",
    ):
        apply_solver_decision(
            state,
            decision,
...
def test_observation_prompt_limits_follow_up_to_work_item_scope() -> None:
    ...
    assert "取得本文に委任があっても、委任された事項のうち" in (
        rendered.instructions
    )
    assert "WorkItemへの回答を\n  確定できない又は必要な限定を示せない内容だけ" in (
        rendered.instructions
    )
    assert "確認できた項目は`resolve_gap_ids[]`" in rendered.instructions
    assert "探索対象に適さない項目は`discard_gap_ids[]`" in rendered.instructions
    assert "新しい未確認事項は、それが未確認だと" in (
        rendered.instructions
    )
...
def test_observation_reconsiders_candidate_matched_by_review() -> None:
    fixture_path = (
        Path(__file__).parent
        / "fixtures"
        / "framework"
...
def test_observation_does_not_revive_candidate_from_discovery_lineage() -> None:
    fixture_path = (
        Path(__file__).parent
        / "fixtures"
        / "framework"
    ...
    candidate = SearchCandidateArticle(
        article_id="law-test-article-no-match",
        document_id="law-test",
        title="周辺規定",
        headings=("周辺規定",),
        discovery_work_item_ids=("wi-3",),
        discovery_hypothesis_ids=("h-3",),
        search_request_ids=("search-1",),
        navigation_evidence_ids=("search-nav-no-match",),
    )
    observation_context = _grounding_observation_context(
        context.model_copy(
            update={
    ...
    assert projected.search_candidates == ()
...
def test_observation_prompt_keeps_question_scoped_delegation_open() -> None:
    ...
    assert "具体化する本文を確認しないとWorkItemへの回答を" in (
        rendered.instructions
    )
    assert "確認できた項目は`resolve_gap_ids[]`" in rendered.instructions
    assert "親規定から具体化規定を探す場合は`from_subject`" in (
...
def test_completion_audit_applies_status_change_and_resets_stale_basis() -> None:
    existing = DependencyDecision(
        dependency_kind="lower_norm",
        work_item_id="wi-1",
        status="resolved",
    ...
    audited = DependencyDecision(
        ...
        work_item_id="wi-1",
        status="not_required",
        reason="この確認事項には下位規範の確認を要しない。",
        basis_evidence_ids=("current",),
    )
    ...
    state = CaseState(
        case_id="case-dependency-status-change",
        question="確認する。",
        work_items=(WorkItem(work_item_id="wi-1", question="確認事項"),),
        hypotheses=(
            Hypothesis(
        ...
        evidence=(
            Evidence(
                evidence_id="parent",
                source_ref="law://parent",
                content="親規定",
                created_cycle=1,
                metadata={"articleId": "article-parent"},
            ),
            Evidence(
                evidence_id="terminal",
                source_ref="law://terminal",
                content="末端規定",
                created_cycle=1,
                metadata={"articleId": "article-terminal"},
            ),
            Evidence(
                evidence_id="current",
                source_ref="law://current",
                content="今回確認した規定",
                created_cycle=1,
                metadata={"articleId": "article-current"},
            ),
        ),
        dependency_decisions=(existing,),
    )
    updated = apply_solver_decision(
        state,
        SolverDecision(
            next="continue",
            dependency_decisions=(audited,),
        ),
        limits=AgentLimits(),
        known_tool_names=set(),
        material_evidence_ids=("current",),
        finalize_only=False,
        allow_dependency_action_without_tool=True,
    )

    assert updated.dependency_decisions == (audited,)
...
def test_observation_prompt_rejects_confirmed_dependency_with_lower_norm_gap(
    ...
    assert "差分適用後も残るgapから下位規範の状態" in (
        rendered.instructions
    )
    assert "`terminal_text_confirmed`と併存させません" in (
        rendered.instructions
    )
    assert rendered.instructions.index("既存の`gaps[]`を1件ずつ本文と照合し") < (
        rendered.instructions.index("差分適用後も残るgapから下位規範の状態")
    )
...
def test_lower_norm_prompt_distinguishes_explicit_delegation_from_guess() -> None:
    prompt_dir = (
    ...
    evidence_prompt = (prompt_dir / "solver_evidence_integration.md").read_text(
        encoding="utf-8"
    )
    dependency_prompt = (
    ...
    assert "委任された事項のうち、具体化する本文を確認しないとWorkItemへの回答を" in (
        evidence_prompt
    )
    assert "確定できない又は必要な限定を示せない内容だけ" in evidence_prompt
...
def test_anthropic_observation_uses_small_json_transport_schema() -> None:
    fixture_path = (
        Path(__file__).parent
        / "fixtures"
        / "framework"
    ...
    openai_rendered = render_observation_integration_model_call(
        context,
        profile,
        provider="openai",
    )
    ...
    assert "Adapterが復元後に" in rendered.instructions
    assert "fetch_articlesとload_evidenceは" in rendered.instructions
    assert rendered.input_payload == openai_rendered.input_payload
    assert rendered.normalized_schema == openai_rendered.normalized_schema
    assert rendered.normalized_schema == (
        ObservationIntegrationDecision.model_json_schema()
    )

    normalized = _normalize_observation_integration_payload(
        {
            "decision_reason": "取得本文を確認事項へ反映した。",
            "update_hypotheses": [],
            "dependency_decisions": [],
            "tool_requests_json": "[]",
            "fetch_articles": None,
    ...
    observation = ObservationIntegrationDecision.model_validate(normalized)
    assert observation.decision_reason == "取得本文を確認事項へ反映した。"

    fetchable_article_id = context.fetchable_article_ids[0]
    normalized_fetch = _normalize_observation_integration_payload(
        {
            "decision_reason": "既知候補の本文を取得する。",
            "update_hypotheses": [],
            "dependency_decisions": [],
            "tool_requests_json": "[]",
            "fetch_articles": {
    ...
    assert normalized_fetch["tool_requests"][0]["arguments"] == {
        "article_ids": [fetchable_article_id]
    }

    normalized_direct_fetch = _normalize_observation_integration_payload(
        {
            "decision_reason": "既知候補の本文を取得する。",
            "update_hypotheses": [],
            "dependency_decisions": [],
    ...
    assert normalized_direct_fetch["tool_requests"][0]["arguments"] == {
        "article_ids": [fetchable_article_id]
    }

    other_article_id = "law-test-article-not-yet-fetched"
    already_fetched_context = context.model_copy(
        update={
            "fetchable_article_ids": (
                fetchable_article_id,
                other_article_id,
            ),
            "fetched_resource_ids_by_work_item_this_cycle": {
                context.work_tree[0].work_item_id: (fetchable_article_id,)
            },
        }
    )
    normalized_partial_refetch = _normalize_observation_integration_payload(
        {
            "decision_reason": "未取得の候補だけ本文を取得する。",
            "update_hypotheses": [],
            "dependency_decisions": [],
            "tool_requests_json": "[]",
            "fetch_articles": {
                "request_id": "fetch-partial-1",
                "work_item_id": context.work_tree[0].work_item_id,
                "purpose": "本文を確認する。",
                "hypothesis_ids": [context.hypotheses[0].hypothesis_id],
                "article_ids": [fetchable_article_id, other_article_id],
            },
            "load_evidence": None,
        },
        context=already_fetched_context,
    )
    assert normalized_partial_refetch["tool_requests"][0]["arguments"] == {
        "article_ids": [other_article_id]
    }

    already_fetched_in_prior_cycle_context = context.model_copy(
        update={
            "fetched_resource_ids_by_work_item": {
                context.work_tree[0].work_item_id: (fetchable_article_id,)
            },
    ...
    normalized_full_refetch = _normalize_observation_integration_payload(
        {
            "decision_reason": "取得済み本文を再利用する。",
            "update_hypotheses": [],
            "dependency_decisions": [
                {
                    "dependency_kind": "lower_norm",
                    ...
                    "reason": "取得済み本文を確認する。",
                    "basis_evidence_ids": [],
                    "action_request_id": "fetch-full-duplicate",
                }
            ],
            "tool_requests_json": "[]",
            "fetch_articles": {
                "request_id": "fetch-full-duplicate",
                "work_item_id": context.work_tree[0].work_item_id,
                "purpose": "本文を確認する。",
                "hypothesis_ids": [context.hypotheses[0].hypothesis_id],
                "article_ids": [fetchable_article_id],
        ...
        context=already_fetched_in_prior_cycle_context,
    )
    assert normalized_full_refetch["tool_requests"] == []
    assert normalized_full_refetch["dependency_decisions"][0][
        "action_request_id"
    ] is None

    normalized_unknown_fetch = _normalize_observation_integration_payload(
        {
            "decision_reason": "未知候補は実行しない。",
            "update_hypotheses": [],
            "dependency_decisions": [],
            "tool_requests_json": "[]",
            "fetch_articles": {
                "request_id": "fetch-unknown-1",
                "work_item_id": context.work_tree[0].work_item_id,
                "purpose": "本文を確認する。",
                "hypothesis_ids": [context.hypotheses[0].hypothesis_id],
                "article_ids": ["invented-article"],
            },
            "load_evidence": None,
        },
        context=context,
    )
    assert normalized_unknown_fetch["tool_requests"] == []

    normalized_generic_fetch = _normalize_observation_integration_payload(
        {
            "decision_reason": "既知候補の本文を取得する。",
            "update_hypotheses": [],
            "dependency_decisions": [],
            "tool_requests_json": json.dumps(
                [
                    {
                        "request_id": "generic-known-fetch",
                        "work_item_id": context.work_tree[0].work_item_id,
                        "tool_name": "fetch_articles",
                        "arguments": {
                            "article_ids": [fetchable_article_id]
    ...
    assert normalized_generic_fetch["tool_requests"][0]["arguments"] == {
        "article_ids": [fetchable_article_id]
    }

    duplicated_fetch = _normalize_observation_integration_payload(
        {
            "decision_reason": "専用欄を正本にする。",
            "update_hypotheses": [],
            "dependency_decisions": [],
            "tool_requests_json": json.dumps(
                [
                    {
                        "request_id": "generic-fetch",
                        "work_item_id": context.work_tree[0].work_item_id,
                        "tool_name": "fetch_articles",
    ...
    assert len(duplicated_fetch["tool_requests"]) == 1
    assert duplicated_fetch["tool_requests"][0]["request_id"] == (
        "dedicated-fetch"
    )

    first_omitted_evidence_id = context.omitted_evidence_ids[0]
    normalized_load = _normalize_observation_integration_payload(
        {
            "decision_reason": "既知Evidenceを再表示する。",
            "update_hypotheses": [],
            "dependency_decisions": [],
            "tool_requests_json": json.dumps(
                [
                    {
                        "request_id": "invented-load",
                        "work_item_id": context.work_tree[0].work_item_id,
                        "tool_name": "load_evidence",
                        "arguments": {"evidence_ids": ["invented-id"]},
                        "purpose": "自由文字列側の要求。",
                        "hypothesis_ids": [
                            context.hypotheses[0].hypothesis_id
                        ],
            ...
            "fetch_articles": None,
            "load_evidence": {
                "request_id": "dedicated-load",
                "work_item_id": context.work_tree[0].work_item_id,
                "purpose": "既知Evidenceを再表示する。",
                "hypothesis_ids": [context.hypotheses[0].hypothesis_id],
                "evidence_ref_1": "e1",
            },
        },
    ...
    assert len(normalized_load["tool_requests"]) == 1
    assert normalized_load["tool_requests"][0]["request_id"] == "dedicated-load"
    assert normalized_load["tool_requests"][0]["arguments"] == {
        "evidence_ids": [first_omitted_evidence_id]
    }

    dropped_unstructured_load = _normalize_observation_integration_payload(
        {
            "decision_reason": "専用欄以外のEvidence IDは採用しない。",
            "update_hypotheses": [],
            "dependency_decisions": [],
            "tool_requests_json": json.dumps(
                [
                    {
                        "request_id": "invalid-load",
                        "work_item_id": context.work_tree[0].work_item_id,
                        "tool_name": "load_evidence",
                        "arguments": {"evidence_ids": ["invented-id"]},
                        "purpose": "未知IDを要求する。",
    ...
    assert dropped_unstructured_load["tool_requests"] == []

    malformed_optional_action = _normalize_observation_integration_payload(
        {
            "decision_reason": "取得本文の評価は完了した。",
            "update_hypotheses": [],
            "dependency_decisions": [],
            "tool_requests_json": '[{"request_id":"cut-off',
            "fetch_articles": None,
            "load_evidence": None,
        },
        context=context,
    )
    assert malformed_optional_action["tool_requests"] == []
    assert "このstepでは実行しない" in (
        malformed_optional_action["decision_reason"]
    )

    hypothesis_with_gap = next(
        item for item in context.hypotheses if item.gaps
    )
    preserved_gap = _normalize_observation_integration_payload(
        {
            "decision_reason": "末端本文は未確認。",
            "update_hypotheses": [
                {
                    "hypothesis_id": hypothesis_with_gap.hypothesis_id,
                    "judgment": "supported",
                    "evidence_ids": [],
                    "gaps": [],
                }
            ...
            "dependency_decisions": [
                {
                    "dependency_kind": "lower_norm",
                    "work_item_id": hypothesis_with_gap.work_item_id,
                    "status": "terminal_text_missing",
                    "reason": "末端本文をまだ確認していない。",
                    "basis_evidence_ids": [],
                    "action_request_id": None,
                }
            ],
    ...
    expected_gap_ids = [
        ...
        for item in structured_hypothesis_gaps(
            hypothesis_with_gap.hypothesis_id,
            hypothesis_with_gap.gaps,
        )
    ]
    assert preserved_gap["update_hypotheses"][0]["discard_gap_ids"] == (
        expected_gap_ids
    )
    assert preserved_gap["update_hypotheses"][0].get("resolve_gap_ids", []) == []
    assert preserved_gap["update_hypotheses"][0]["add_gaps"] == []
...
def test_lr_003_cycle_close_fixture_requires_every_deferred_frontier_resolution() -> None:
    ...
    assert "`active_deferred_frontiers`は全件を一度ずつ処理" in call.request
    assert "`active_deferred_frontiers`の全IDを1回ずつ" in call.request


def test_cycle_close_mechanically_closes_deferred_frontier_for_resolved_work(
) -> None:
    state = CaseState(
        case_id="cycle-close-resolved-deferred",
        question="二つの事項を確認する。",
        research_cycle_count=1,
        work_items=(
            WorkItem(
            ...
            WorkItem(work_item_id="wi-2", question="未確認事項"),
        ),
        hypotheses=(
            Hypothesis(
                hypothesis_id="h-1",
                work_item_id="wi-1",
                statement="確認済み事項には根拠がある。",
            ),
            Hypothesis(
                hypothesis_id="h-2",
                work_item_id="wi-2",
                statement="未確認事項には根拠がある。",
            ),
        ),
    )
    context = build_solver_context(
        state,
        AgentLimits(),
        remaining_wall_time_sec=300,
        finalize_only=False,
    ).model_copy(
        update={
            "graph_review_ledger": (
                GraphReviewLedgerItem(
                    frontier_item_id="frontier-closed-work",
                    article_id="article-2",
                    title="検証規則",
                    heading="第二条",
                    work_item_id="wi-1",
    ...
    observation = ObservationIntegrationDecision(
        decision_reason="Cycle内の本文は統合済み。",
        update_work_items=(
            WorkItemUpdate(
                work_item_id="wi-1",
                state="resolved",
    ...
    llm_frontiers, automatic_frontiers = _cycle_close_deferred_frontiers(
        context,
        observation,
    )
    rendered = render_cycle_close_model_call(
    ...
    decision = _normalize_cycle_close_decisions(
        ...
        CycleCloseDecision(
            ...
            next_focus_work_item_ids=("wi-2",),
        ),
    )

    assert llm_frontiers == ()
    assert [item.frontier_item_id for item in automatic_frontiers] == [
        "frontier-closed-work"
    ]
    assert rendered.input_payload["active_deferred_frontiers"] == []
    assert "deferred_frontier_resolutions" not in rendered.output_schema[
        "properties"
    ]
    assert len(decision.deferred_frontier_resolutions) == 1
    assert decision.deferred_frontier_resolutions[0].action == (
        "no_longer_needed"
    )


def test_cycle_close_mechanically_carries_omitted_active_deferred_frontier(
) -> None:
    state = CaseState(
        case_id="cycle-close-carry-omitted-deferred",
        question="未確認事項を次Cycleで確認する。",
        research_cycle_count=1,
        work_items=(WorkItem(work_item_id="wi-1", question="未確認事項"),),
        hypotheses=(
            Hypothesis(
                hypothesis_id="h-1",
                work_item_id="wi-1",
    ...
    context = build_solver_context(
        ...
        update={
            "graph_review_ledger": (
                GraphReviewLedgerItem(
                    frontier_item_id="frontier-active",
                    article_id="article-2",
                    title="検証規則",
                    heading="第二条",
                    work_item_id="wi-1",
    ...
    observation = ObservationIntegrationDecision(
        decision_reason="現在Cycleの本文を統合した。",
    )
    ...
    assert len(decision.deferred_frontier_resolutions) == 1
    resolution = decision.deferred_frontier_resolutions[0]
    assert resolution.frontier_item_id == "frontier-active"
    assert resolution.action == "carry_forward"


def test_completed_current_exploration_set_uses_cycle_close_for_deferred_frontier(
) -> None:
    state = CaseState(
        case_id="completed-set-with-deferred-frontier",
        question="未確認事項を次Cycleで確認する。",
        research_cycle_count=1,
        work_items=(WorkItem(work_item_id="wi-1", question="未確認事項"),),
        hypotheses=(
        ...
        tool_requests=(
            ToolRequest(
                ...
                work_item_id="wi-1",
                tool_name="legal_search",
                arguments={"query": "確認対象"},
                purpose="起点Articleを探す。",
                hypothesis_ids=("h-1",),
            ),
            ToolRequest(
                request_id="graph-1",
    ...
    assert context.graph_review_batch.candidates == ()
    assert context.graph_review_batch.remaining_unreviewed_count == 5
    assert _has_pending_known_candidate_processing(context) is False
    assert context.hypothesis_exploration_sets[0].remaining_new_sets_total == 1
    assert _current_cycle_exploration_set_complete(context) is True

    projected = _project_completed_exploration_set_to_cycle_close(
        context,
        finalize_only=False,
        pending_grounding_observation=False,
        pending_known_candidate_processing=False,
        revision_ready=False,
    )

    assert projected.cycle_close_required is True
    ...
    assert purpose == "cycle_close"

    decision = _normalize_cycle_close_decisions(
        projected,
        ObservationIntegrationDecision(
            decision_reason="現在Cycleの本文は統合済み。",
        ),
        CycleCloseDecision(
            decision_reason="未確認事項を次Cycleへ引き継ぐ。",
            next_focus_work_item_ids=("wi-1",),
    ...
    assert [
        (item.frontier_item_id, item.action)
        for item in decision.deferred_frontier_resolutions
    ] == [("frontier-active", "carry_forward")]

    fallback = normalize_dependency_action_decision(
        {
            "decision_reason": "現在Cycleに有効な追加探索がない。",
            "start_next_cycle": True,
            "tool_requests": [],
        },
        context=context,
    ...
    assert [
        (item.frontier_item_id, item.action)
        for item in fallback.deferred_frontier_resolutions
    ] == [("frontier-active", "carry_forward")]
    applied = apply_solver_decision(
        state,
        fallback,
        ...
        require_dependency_decisions=True,
        deferred_frontiers={
            "frontier-active": ("article-2", "wi-1", "h-1")
        },
        unreviewed_graph_candidate_count=5,
    ...
    assert applied.deferred_frontier_resolutions[-1].action == "carry_forward"


@pytest.mark.parametrize(
    "pending",
    ("grounding", "candidate", "revision"),
)
def test_completed_current_exploration_set_waits_for_pending_processing(
    pending: str,
) -> None:
    context = build_solver_context(
        CaseState(
            case_id=f"completed-set-pending-{pending}",
            question="未確認事項を確認する。",
            research_cycle_count=1,
            work_items=(WorkItem(work_item_id="wi-1", question="未確認事項"),),
            hypotheses=(
            ...
            tool_requests=(
                ...
                ToolRequest(
                    ...
                    work_item_id="wi-1",
                    tool_name="legal_graph_neighbors",
                    purpose="関係Articleを探す。",
                    hypothesis_ids=("h-1",),
                ),
            ),
            tool_results=(
                ToolResult(request_id="search-1", status="succeeded", cycle_no=1),
    ...
    projected = _project_completed_exploration_set_to_cycle_close(
        context,
        finalize_only=False,
        pending_grounding_observation=pending == "grounding",
        pending_known_candidate_processing=pending == "candidate",
        revision_ready=pending == "revision",
    )

    assert projected.cycle_close_required is False


def test_non_boundary_step_drops_deferred_frontier_resolutions() -> None:
    context = build_solver_context(
        CaseState(case_id="case-non-boundary", question="確認する。"),
        AgentLimits(),
        remaining_wall_time_sec=300,
        finalize_only=False,
    )
    ...
    assert normalized["deferred_frontier_resolutions"] == []
    assert normalized["unreviewed_graph_resolution"] is None


def test_supported_hypothesis_delta_can_reuse_existing_evidence() -> None:
    context = build_solver_context(
        CaseState(
            case_id="case-supported-delta",
            question="確認する。",
            work_items=(WorkItem(work_item_id="wi-1", question="確認事項"),),
            hypotheses=(
                Hypothesis(
    ...
    decision = SolverDecision(
        next="continue",
        decision_reason="既存の支持判断を維持し、未確認事項だけ追加する。",
        update={
            "update_hypotheses": [
                {
                    "hypothesis_id": "h-1",
                    "judgment": "supported",
                    "evidence_ids": [],
                    "add_gaps": [{"description": "追加で確認する具体的条件"}],
                    "resolve_gap_ids": [],
                }
            ]
        },
...
def test_finalization_mechanically_closes_deferred_frontier_for_resolved_work(
) -> None:
    state = CaseState(
        case_id="finalization-resolved-deferred",
        question="確認済み事項へ回答する。",
        research_cycle_count=1,
        work_items=(
            WorkItem(
                work_item_id="wi-1",
...
def test_fetch_normalization_preserves_independent_work_item_capacity() -> None:
    fixture_path = (
        Path(__file__).parent
        / "fixtures"
        / "framework"
    ...
    normalized = {
        "next": "continue",
        "dependency_decisions": [
            {"work_item_id": "wi-2", "action_request_id": "fetch-2"}
        ],
        "tool_requests": [
            {
                "request_id": "fetch-1",
            ...
            },
            {
                ...
                "work_item_id": "wi-2",
                "tool_name": "fetch_articles",
                "arguments": {"article_ids": ["article-2", "article-3"]},
                "hypothesis_ids": ["h-2"],
            },
        ],
    }
    ...
    assert [
        request["request_id"] for request in normalized["tool_requests"]
    ] == ["fetch-1", "fetch-2"]
    assert normalized["dependency_decisions"][0]["action_request_id"] == "fetch-2"
...
def test_tob_announcement_cycle_close_must_continue_for_projected_open_work(
    ...
    assert "answer" not in rendered.output_schema["properties"]
...
def test_integration_finalize_drops_next_cycle_evidence_retention() -> None:
    context = build_solver_context(
        CaseState(case_id="integration-finalize-retention", question="質問"),
        AgentLimits(max_retained_evidence=1),
        remaining_wall_time_sec=300,
        finalize_only=False,
    )
    normalized = {
        "next": "finalize",
        "retain_evidence_ids": ["e-1", "e-2"],
    }

    _normalize_absent_context_branches(normalized, context)

    assert normalized["retain_evidence_ids"] == []
...
def test_finalization_mechanically_keeps_resolved_work_item_basis_ids() -> None:
    ...
    context = build_solver_context(
        ...
        update={"grounding_evidence_ids": ("e1", "e-unbound")}
    )
    payload = {
        "answer": {
            "text": "確認済み事項を回答する。",
...
def test_integration_phase_finalization_keeps_required_citations() -> None:
    state = CaseState(
        case_id="integration-finalization-citations",
        question="確認済み事項を回答する。",
        research_cycle_count=1,
        work_items=(
            WorkItem(
    ...
    class IntegrationFinalizationLLM:
        provider = "anthropic"
    ...
    result = StructuredJSONModelAdapter(IntegrationFinalizationLLM()).solve(
        context,
        ModelCallProfile(
            model="test-model",
            system_prompt="確認済み事項を回答する。",
            context_projection="full",
        ),
    )

    assert result.decision.answer is not None
    assert result.decision.answer.citation_ids == ("e1",)
...
def test_cycle_close_fixture_projects_three_small_single_task_calls() -> None:
    ...
    assert update_schema["discard_gap_ids"]["items"]["enum"]

    action_context = context.model_copy(
        update={
            "cycle_close_required": False,
            "remaining_fetch_capacity": 15,
            "remaining_fetch_capacity_by_work_item": {
                item.work_item_id: 3 for item in context.work_tree
            },
            "search_candidates": tuple(
                candidate.model_copy(
                    update={
    ...
    assert [item.remaining_fetch_capacity for item in allocated] == [3, 3, 3, 3, 3]
    assert all(
    ...
    assert "available_tools" in observation_call.input_payload
...
def test_cycle_close_keeps_dependency_status_without_answer_evidence() -> None:
    fixture_path = (
        Path(__file__).parent
        / "fixtures"
        / "framework"
    ...
    assert "grounding_evidence" not in rendered.input_payload
    assert "required_answer_evidence_ids" not in rendered.input_payload
...
def test_observation_step_can_consume_an_irrelevant_article_without_a_fake_update(
) -> None:
    evidence = Evidence(
        evidence_id="e-1",
        source_ref="fixture:e-1",
        content="この本文は現在の未確認事項を扱わない。",
        created_cycle=2,
        metadata={"articleId": "article-1", "citationEligible": True},
    )
    state = CaseState(
        case_id="fixture-observation-no-diff",
        question="要件を確認する。",
        research_cycle_count=2,
        work_items=(WorkItem(work_item_id="wi-1", question="要件は何か。"),),
        hypotheses=(
            Hypothesis(
                hypothesis_id="h-1",
                work_item_id="wi-1",
    ...
    class NoDiffObservationLLM:
        provider = "openai"

        def generate_structured_json(self, **kwargs: Any) -> StructuredJSONResult:
            return StructuredJSONResult(
                payload={
                    "decision_reason": "本文は未確認事項を扱わない。",
                    "update_hypotheses": [],
                    "dependency_decisions": [],
                    "tool_requests": [],
                },
    ...
    profile = legal_profiles.legal_agent_profile().solver_observation_integration
    assert profile is not None
    result = StructuredJSONModelAdapter(NoDiffObservationLLM()).solve(
        context,
        profile,
    )
...
def test_observation_step_can_choose_the_immediate_next_tool() -> None:
    ...
    full_context = full_context.model_copy(
        update={
            "search_candidates": tuple(
                candidate.model_copy(
                    update={
                ...
                for candidate in full_context.search_candidates
            )
        }
    )
            ...
            "update": observed.update.model_copy(
                update={
                    ...
                    "update_hypotheses": tuple(
                        item.model_copy(
                            update={
                                "legacy_replacement_gaps": (
                                    item.legacy_replacement_gaps
                                    or ("未確認の下位規範の具体的内容",)
                                )
                            }
                        )
...
def test_tool_request_schema_is_empty_when_no_tool_is_available() -> None:
    context = build_solver_context(
        CaseState(case_id="case-1", question="質問"),
        AgentLimits(),
        remaining_wall_time_sec=120,
...
def test_anthropic_transport_uses_one_fixed_slot_article_fetch() -> None:
    ...
    assert fetch_object["properties"]["article_ids"]["items"]["enum"] == [
        "article-long-1",
        "article-long-2",
        "article-long-3",
        "article-long-4",
...
def test_dependency_action_uses_dedicated_contract_and_preserves_prior_decision(
    ...
    presented_candidate_ids = set(
        fetch_schema["items"]["properties"]["article_ids"]["items"]["enum"]
    )
    assert presented_candidate_ids == expected_candidate_ids
    ...
    anthropic_decision = normalize_dependency_action_decision(
        {
            ...
            "tool_requests_json": json.dumps(
                [
                    {
                        "request_id": "fetch-wire-fallback",
                        "work_item_id": work_item_id,
                        "tool_name": "fetch_articles",
                        "arguments": {
                            "article_ids": [fetchable_candidate.article_id],
                        },
                        "purpose": "未確認候補の本文を確認する。",
                        "hypothesis_ids": [hypothesis_id],
                    }
                ],
                ensure_ascii=False,
    ...
    assert anthropic_decision.tool_requests[0].tool_name == "fetch_articles"
    assert anthropic_decision.tool_requests[0].arguments == {
        "article_ids": [fetchable_candidate.article_id]
    }
...
def test_rejected_dependency_action_forces_the_next_cycle_contract() -> None:
    ...
    anthropic_wire_decision = normalize_dependency_action_decision(
        {
            "decision_reason": (
                "利用可能なToolがないため、現在Cycleでは続行できない。"
            ),
            "start_next_cycle": False,
            "tool_requests_json": "[]",
        },
    ...
    assert anthropic_wire_decision.start_next_cycle is True
    assert anthropic_wire_decision.tool_requests == ()


def test_dependency_action_carries_unreviewed_graph_pool_to_next_cycle() -> None:
    state = CaseState(
        case_id="case-dependency-action-graph-pool",
        question="質問",
        research_cycle_count=1,
        work_items=(
            WorkItem(work_item_id="wi-1", question="確認事項"),
        ),
        hypotheses=(
            Hypothesis(
                hypothesis_id="h-1",
    ...
    decision = normalize_dependency_action_decision(
        {
            "decision_reason": "現在Cycleに有効な行動がないため見直す。",
            "start_next_cycle": True,
            "tool_requests": [],
        },
        context=context,
    ...
    assert decision.unreviewed_graph_resolution is not None
    assert decision.unreviewed_graph_resolution.action == "review_next_cycle"


def test_dependency_action_maps_content_unit_evidence_to_its_article_root() -> None:
    evidence = Evidence(
        evidence_id="law-a-article-2-paragraph-1-item-3",
        source_ref="fixture:item-3",
        content="第三号の本文",
        title="テスト法",
        created_cycle=1,
        metadata={
            "articleId": "law-a-article-2",
            "sourceContentUnitId": "law-a-article-2-paragraph-1-item-3",
            "evidenceRole": "retrieved_text",
            "citationEligible": True,
        },
    )
    state = CaseState(
        case_id="case-content-unit-graph-root",
        question="下位規範を確認する。",
        research_cycle_count=1,
        work_items=(WorkItem(work_item_id="w1", question="基準を確認する。"),),
        hypotheses=(
            Hypothesis(
                hypothesis_id="h1",
                work_item_id="w1",
                statement="基準は下位規範で定められる。",
    ...
    context = build_solver_context(
        ...
        required_dependency_kind="lower_norm",
        required_dependency_work_item_ids=("w1",),
    ).model_copy(update={"available_tools": (LegalGraphNeighborsTool.definition,)})

    decision = normalize_dependency_action_decision(
        {
            "decision_reason": "第三号が参照する下位規範を確認する。",
            "start_next_cycle": False,
            "tool_requests_json": json.dumps(
                [
                    {
                        ...
                        "tool_name": "legal_graph_neighbors",
                        "arguments": {
                            "article_ids": [evidence.evidence_id],
                            "max_relations": 10,
                            "mode": "reference_edges",
                            "predicate": None,
                            "reference_lookup": "follow_reference_in_text",
                        },
                        "purpose": "第三号に明記された参照先を確認する。",
                        "hypothesis_ids": ["h1"],
                    }
                ]
            ),
    ...
    assert decision.tool_requests[0].arguments["article_ids"] == [
        "law-a-article-2"
    ]
    updated = apply_solver_decision(
        state,
        ...
        graph_known_article_ids={"law-a-article-2"},
        required_dependency_kind="lower_norm",
        required_dependency_work_item_ids=("w1",),
        require_dependency_decisions=True,
        allow_dependency_action_without_tool=True,
    ...
    assert updated.tool_requests[-1].arguments["article_ids"] == [
        "law-a-article-2"
    ]
...
def test_anthropic_generic_fetch_slot_is_ignored_when_sidecar_is_null() -> None:
    payload = {
        "next": "continue",
        "update_json": "{}",
        "hypothesis_evidence_bindings": [],
...
def test_anthropic_generic_article_fetch_alias_is_ignored() -> None:
    payload = {
        "next": "continue",
        "update_json": "{}",
        "hypothesis_evidence_bindings": [],
...
def test_anthropic_json_transport_uses_known_article_fetch_sidecar() -> None:
    state = CaseState(
        case_id="case-anthropic-fetch-sidecar",
        question="確認する。",
        research_cycle_count=1,
        work_items=(WorkItem(work_item_id="wi-1", question="本文を確認する。"),),
        hypotheses=(
            Hypothesis(
                hypothesis_id="h-1",
                work_item_id="wi-1",
    ...
    context = build_solver_context(
        ...
        update={
            "fetchable_article_ids": ("article-known",),
            "remaining_fetch_capacity": 1,
        }
    )
    rendered = render_solver_model_call(
    ...
    assert "dedicated fetch_articles field" in rendered.output_schema[
        "properties"
    ]["decision_json"]["description"]
    assert rendered.output_schema["properties"]["fetch_articles"]["anyOf"][0][
        "properties"
    ]["article_ids"]["items"]["enum"] == ["article-known"]
    ...
    normalized = _normalize_solver_payload(
        {
            ...
            "fetch_articles": {
                "request_id": "known-fetch",
                "work_item_id": "wi-1",
                "purpose": "既知候補を取得する。",
                "hypothesis_ids": ["h-1"],
                "article_ids": ["article-known"],
            },
        }
    )
    ...
    assert normalized["tool_requests"] == [
        {
            ...
            "work_item_id": "wi-1",
            "tool_name": "fetch_articles",
            "arguments": {"article_ids": ["article-known"]},
            "purpose": "既知候補を取得する。",
            "hypothesis_ids": ["h-1"],
        }
    ]
...
def test_overview_finalization_projects_verified_open_work_material() -> None:
    ...
    state = state.model_copy(
        update={
            "evidence": (
                *state.evidence,
                Evidence(
                    evidence_id="e-fetched-but-unlinked",
                    source_ref="fixture:e-fetched-but-unlinked",
                    content="取得したArticle内の別の規定内容。",
                    created_cycle=2,
                    metadata={
                        "articleId": "article-unlinked",
                        "citationEligible": True,
                    },
                ),
            ),
    ...
    assert all(
        set(item) == {"evidence_id", "content", "title"}
        for item in rendered.input_payload["material_evidence"]
    )
    assert set(rendered.input_payload["grounding_evidence_ids"]) == {
        item["evidence_id"]
        for item in rendered.input_payload["material_evidence"]
    }
    assert rendered.input_payload["grounding_evidence_ids"] == [
...
def test_article_fetched_by_another_work_item_is_not_removed() -> None:
    state = CaseState(
        case_id="case-cross-work-item-fetch",
        question="質問",
        research_cycle_count=1,
        work_items=(
            WorkItem(work_item_id="w1", question="条件を確認する"),
            WorkItem(work_item_id="w2", question="例外を確認する"),
        ),
        hypotheses=(
            Hypothesis(
                hypothesis_id="h1",
        ...
        tool_requests=(
            ToolRequest(
                ...
                work_item_id="w1",
                tool_name="fetch_articles",
                arguments={"article_ids": ["article-shared"]},
                purpose="本文を確認する",
                hypothesis_ids=("h1",),
            ),
        ),
