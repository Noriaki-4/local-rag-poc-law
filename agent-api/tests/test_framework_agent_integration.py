...
class FakeStructuredLLM:
    ...
    def __init__(self) -> None:
        ...
        self.payloads: list[dict[str, Any]] = [
            ...
            {
                "decision_reason": "取得本文に下位規範への委任はない",
...
def test_new_framework_uses_legal_tool_and_skips_reviewer_by_default(
    ...
    assessment_schema = search_schema["properties"]["assessments"]
    assert assessment_schema["type"] == "object"
    assert len(assessment_schema["properties"]) == 1
    assert assessment_schema["required"] == list(
        assessment_schema["properties"]
    )
    assert set(llm.calls[4]["schema"]["properties"]) == {"roles"}
...
def test_legal_solver_prompts_are_projected_by_structural_mode() -> None:
    ...
    assert "# 法令調査Solver：取得本文の評価" in cycle_close_prompt
    assert "次の検索、Cycle移行、最終回答を決めません" in cycle_close_prompt
    assert "grounding_evidence" in cycle_close_prompt
    assert "主体、行為、対象、条件" in cycle_close_prompt
    dependency_prompt = profile.solver_cycle_close.dependency_system_prompt
    assert dependency_prompt is not None
    assert "上位規範と、その内容を補完する末端下位規範" in dependency_prompt
    assert "複数階層の規範が同じ法的論点" in dependency_prompt
    assert "例：法律の原則を政令が具体化" in dependency_prompt
...
def test_overtime_hypothesis_gap_failure_fixture_tracks_the_contract_fix() -> None:
    ...
    assert "誤り得る暫定的な結論" in statement_description
    assert "法令本文によって支持または否定できる" in statement_description
    assert "WorkItemの言い換えだけにはせず" in statement_description
    assert "未確認の規律要素" in gaps_description
...
def test_search_review_duplicate_fixture_uses_article_keyed_assessments() -> None:
    ...
    assert prompt.index("## 出力前の確認") > prompt.index(
        "</solver_context>"
    )
...
def test_search_reselection_underselect_fixture_checks_remaining_hypotheses(
    ...
    assert "主体一致を確認" in prompt
    assert "行為、対象、条件" in prompt
...
def test_search_reselection_rejects_candidate_without_hypothesis_match() -> None:
    ...
    with pytest.raises(
        ModelProtocolError,
        match="must directly match a hypothesis",
    ):
        _validate_search_reselection_payload(
            {
                "selections": [
...
def test_real_search_actor_mismatch_is_preserved_as_fixture() -> None:
    ...
    assert "matched_hypothesis_ids" in assessment_item["properties"]
    assert "規律主体の照合や候補選択をしません" in rendered.instructions
    assert "見出しが規律主体や対象範囲を限定" in rendered.instructions
    assert "Hypothesisの`statement`か`gaps`" in rendered.instructions
    assert "規律主体をいったん除き" in assessment_item[
        "properties"
    ]["matched_hypothesis_ids"]["description"]
...
def test_search_actor_classification_intersects_content_and_actor_matches() -> None:
    fixture_path = (
        Path(__file__).parent
        / "fixtures"
        / "framework"
    ...
    assert "法的機能、条件、効果、候補の採否は判断しません" in (
...
def test_candidate_selection_profiles_have_post_context_checks() -> None:
    ...
    assert assessment_prompt.rindex("## 出力前の確認") > (
...
def test_cycle_close_prompts_separate_observation_from_transition() -> None:
    ...
    assert "既存のWorkItemとHypothesis" in observation
...
def test_finalization_schema_rejects_stale_deferred_frontier_ids() -> None:
    ...
    frontier_schema = rendered.output_schema["properties"][
        "deferred_frontier_resolutions"
    ]["items"]["properties"]["frontier_item_id"]

    assert frontier_schema["enum"] == active_ids
...
def test_observation_projects_article_to_hypothesis_candidate_links() -> None:
    ...
    assert "対応候補であり、支持・反証を確定するものではありません" in (
        rendered.instructions
    )
...
def test_graph_review_can_select_fetched_article_without_refetching() -> None:
    fixture_path = (
        Path(__file__).parent
        / "fixtures"
        / "framework"
...
def test_observation_keeps_each_work_item_dimension_separate() -> None:
    ...
    assert "範囲を確認するWorkItemへ、例外・成立条件・手続を" in (
        observation_call.instructions
    )
    assert "Hypothesisの文を読み替えて`supported`" in (
        observation_call.instructions
    )
    assert "別WorkItemの条件・例外・手続を付け足しません" in (
        observation_call.instructions
    )
    ...
    assert "そのWorkItemの`question`に関係する範囲に限り" in (
...
def test_tob_final_cycle_requires_all_deferred_frontiers_in_both_schemas() -> None:
    ...
    assert final_resolutions["minItems"] == active_count
    assert final_resolutions["maxItems"] == active_count
...
def test_cycle_close_fixture_projects_three_small_single_task_calls() -> None:
    ...
    assert set(dependency_call.input_payload) == {
        "question",
    ...
    assert set(dependency_call.output_schema["properties"]) == {
        "decision_reason",
        "dependency_decisions",
    }
    dependency = DependencyAssessmentDecision.model_validate(
        {
            "decision_reason": "末端下位規範本文が不足している",
            "dependency_decisions": [
                {
                    "dependency_kind": "lower_norm",
...
def test_cycle_close_adapter_combines_observation_and_transition() -> None:
    ...
    assert len(llm.calls) == observation_count + 2
    assert result.attempt_count == observation_count + 2
...
def test_cycle_close_skips_dependency_call_when_no_work_item_requires_it() -> None:
    ...
    assert len(llm.calls) == observation_count + 1
    assert result.attempt_count == observation_count + 1
    assert result.decision.start_next_cycle is True
...
def test_real_observation_fixture_preserves_evidence_hypothesis_provenance(
    ...
    assert "確認に使ったEvidence IDを残した" in rendered.instructions
...
def test_observation_integration_projects_only_open_work() -> None:
    ...
    assert "入力にない完了済みWorkItem" in rendered.instructions
...
def test_cycle_close_cannot_finalize_needs_action_dependency() -> None:
    ...
    aligned = _align_observation_with_dependency_decisions(observed)

    work_item_update = aligned.update_work_items[0]
    assert work_item_update.state == "open"
    assert work_item_update.resolution is None
    assert work_item_update.basis_hypothesis_ids == ()
...
def test_resolved_work_with_open_dependency_fixture_is_rejected() -> None:
    ...
    profile = legal_profiles.legal_agent_profile().solver_cycle_close
    assert profile is not None
    assert "そのWorkItemを`resolved`にせず`open`へ戻します" in (
        profile.system_prompt
    )
...
def test_finalization_projects_only_resolved_work_item_evidence() -> None:
    fixture_path = (
        Path(__file__).parent
        / "fixtures"
        / "framework"
    ...
    assert rendered.input_payload["navigation_evidence_ids"] == []
    assert "`resolved_work_item_ids=[]`なら" in (
        rendered.instructions
    )
...
def test_finalization_rejects_citations_without_resolved_work_item_basis() -> None:
    ...
    with pytest.raises(
        ContractViolation,
        match="citations require resolved WorkItem basis",
    ):
        apply_solver_decision(
            state,
            decision,
