...
1. `search_candidate_checklist.article_ids_in_input_order`を、同じ順序で`assessments.article_id`へ1回ずつ写します。
2. 全候補を入力順に読みます。
3. 各候補について、規定が成立する条件と、そのとき生じる効果を一文にまとめます。
4. 条件ではなく主な効果を基準に、法的機能を次の4値から一つ選びます。
5. 要約と法的機能が矛盾していないか確認します。
...
- `assessments`: チェックリストのArticle IDを同じ順序でちょうど1回返します。`legal_function`、条件と効果を自分の言葉でまとめた`summary`を付けます。
