...
- `assessments`：全候補の内容要約、法的機能、対応Hypothesis、満たせる回答要求
...
- `work_tree`: 現在の作業。
- `hypotheses`: 検証する命題。
- `non_work_item_requirements`: 根拠条文の提示等、回答全体で守る明示要求。
...
2. 各候補について、規定の主体、行為、対象、成立条件、効果を一文にまとめます。
   根拠法令・条文の提示が要求され、抜粋が別Articleを根拠として明示している場合は、
   その参照先も要約に残します。
...
4. 候補本文の取得によって`non_work_item_requirements`を満たせる場合は、その要求を
   `matched_non_work_item_requirements`へ入力どおりに入れます。
   根拠法令・条文の提示では、義務を直接定める規定と、それを具体化する詳細規定の両方を候補にできます。
   ただし、参照先の本文確認は参照元だけで代用しません。
5. 条件ではなく主な効果を基準に、法的機能を次の4値から一つ選びます。
6. 要約、対応Hypothesis、回答全体の要求、法的機能が矛盾していないか確認します。
