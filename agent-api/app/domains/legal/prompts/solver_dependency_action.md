...
3. `hypothesis_exploration_sets[]`で、対象Hypothesisが現在セットで使える探索を確認します。
4. 取得本文が、WorkItemで問われた未確認内容を下位規範へ明示的に委ねている場合は、その委任元Articleから下位規範を探します。
5. それ以外は、本文未取得の既知候補が未確認事項を直接扱うか確認します。該当する候補があれば本文取得を選びます。
6. 該当する既知候補がなければ、次の判断基準から未確認事項を最も直接進めるToolを選びます。
...
7. 有効な行動がある場合は、処理上限内で今回進める`needs_action` WorkItemを選び、各WorkItemにToolRequestを1件返して`start_next_cycle=false`にします。今回選ばないWorkItemは次stepへ残します。
8. 成功済みscope以外に未確認事項を進める行動がない場合は、ToolRequestを返さず`start_next_cycle=true`にします。
