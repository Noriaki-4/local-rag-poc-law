...
- 下位規範を追うのは、取得本文がWorkItemの未確認事項を別規範へ明示的に委任している場合だけです。
  `terminal_text_missing`の`reason`には、本文中の委任と委任事項を示します。示せなければ選びません。
- 「府令で定めるところにより」等の委任句が、WorkItemで問う行為、条件、範囲又は方法を修飾する場合は、
  その委任事項も確認事項に含めます。上位本文が選択肢又は義務を示すだけでは`not_required`にしません。
- 取得本文がWorkItemへ直接答え、同じ事項を別規範へ委任していなければ`not_required`にします。
  Hypothesisで予想した詳細や、該当する特則が存在する可能性だけを理由に下位規範を追いません。
- 質問やWorkItemに複数の法令種別が候補として示されていても、取得本文にない委任先を予想しません。
- 関係規定又は改正影響先の列挙では、候補Article本文と起点規定との関係を確認できれば、
  候補Article内のさらなる参照先は、その関係分類に必要な場合だけ追います。
...
- 異なるArticleが複数あるだけでは`terminal_text_confirmed`にしません。各規範が同じ法的論点を段階的に
  定める関係であることを確認します。
- `terminal_text_confirmed`は、本文に明示された同じ確認事項の委任が、提示された具体化規定の
  本文へ到達している場合だけ選びます。
...
- 具体化規定が同じ事項をさらに明示的に委任している場合も、委任先本文を確認するまで
  `terminal_text_missing`です。
- 末端候補が別の根拠条文、行為又は手続段階を明示する場合は、同じ系列として扱いません。
- 本文が明示的に委任した具体的な条件、範囲又は手続を質問が求める場合、下位規範の存在を示すだけの
  本文を末端本文としません。
